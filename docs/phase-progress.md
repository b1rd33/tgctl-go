...
| 5. Add everyday account management | implemented (offline first increment) | Archive/unarchive via typed `folders.editPeerFolders` peer requests; folder IDs 1/0, one-target selection, account isolation, write/read-only/fuzzy/dry-run gates, durable idempotency, cancellation and committed-persistence failure handling, no custom-folder/pin/read-marker side effects | pending archive branch commit | Live archive/unarchive acceptance remains unauthorized; mute/unmute, contact add/remove, formatted text/captions, and logout remain separate increments |
...
passed. The archive/unarchive increment is offline-only and does not include
